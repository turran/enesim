...
#else
static Eina_Bool enesim_image_jpg_provider_init(void)
#endif
{
	...
	if (!enesim_image_provider_register(&_provider,
			ENESIM_PRIORITY_PRIMARY, "image/jpg"))
		return EINA_FALSE;
	/* some define the mime type for jpg as jpeg, or is the other
	 * the wrong one ? */
	if (!enesim_image_provider_register(&_provider,
			ENESIM_PRIORITY_PRIMARY, "image/jpeg"))
		return EINA_FALSE;
...
#else
static void enesim_image_jpg_provider_shutdown(void)
#endif
{
